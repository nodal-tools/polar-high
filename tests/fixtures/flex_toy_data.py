...
def make_flex_toy_data(
    *,
    ...
    flow_from_commodity_eff = (
        ...
        .join(commodity_node, left_on="source", right_on="n", how="inner")
        .select("p", "source", "sink", "c")
    )

    # parameters
    slope_value = 1.0 / gt_efficiency
    slope = Param(("p", "t"), pl.DataFrame({
        "p":     ["GT"]   * 4 + ["WIND"] * 4,
        "t":     ts * 2,
    ...
    return FlexToyData(
        ...
        cap       = Param(("p",), pl.DataFrame({"p": ["GT", "WIND"],
                                                  "value": [gt_capacity, wind_capacity]})),
        slope     = slope,
        wind_avail = Param(("t",), pl.DataFrame({"t": ts,
                                                   "value": list(wind_avail_values)})),
        demand    = Param(("n", "t"), pl.DataFrame({
            "n": ["elec"] * 4, "t": ts, "value": list(demand_values),
        })),
        gas_price = Param(("c", "t"), pl.DataFrame({
            "c": ["GAS_COM"] * 4, "t": ts, "value": [gas_price_value] * 4,
        })),
