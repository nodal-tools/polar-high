...
def _build_synthetic_problem(cost: np.ndarray, demand: np.ndarray) -> fp.Problem:
    ...
    lag = pl.DataFrame(
        {"t": np.arange(1, _N_T, dtype=np.int64), "t_prev": np.arange(0, _N_T - 1, dtype=np.int64)}
    )
    s_lag = fp.Lag(v_state, lag, time_dim="t", lag_col="t_prev")

    demand_p = fp.Param(
        ("t",), pl.DataFrame({"t": np.arange(_N_T, dtype=np.int64), "value": demand})
    )
    p.add_cstr(
        "balance",
        over=t_idx,
    ...
    cost_p = fp.Param(("t",), pl.DataFrame({"t": np.arange(_N_T, dtype=np.int64), "value": cost}))
...
def warm_run_synthetic(costs: np.ndarray, demands: np.ndarray) -> dict:
    ...
    for r in range(1, n_rolls):
        ...
        h.changeRowsBounds(_N_T, balance_row_idx.astype(np.int32), new_rhs, new_rhs)
        # Update cost vector for v_flow columns.
        h.changeColsCost(_N_T, v_flow_col_idx.astype(np.int32), costs[r].astype(np.float64))
...
def _build_and_get_warm_handles(problem: fp.Problem):
    ...
    for name, proto, over in problem._cstrs:
        ...
        next_row += row_count
        if name == "balance":
            balance_row_idx = np.arange(base_row, base_row + row_count, dtype=np.int64)
        ...
        for term in expr.terms:
            if term.dims:
                ...
                pending.append(("scalar", base_row, row_count, term.lazy.select("col_id", "coef")))
    ...
    rows_lb_arr = (
        np.concatenate(rows_lb_chunks) if rows_lb_chunks else np.zeros(0, dtype=np.float64)
    )
    rows_ub_arr = (
        np.concatenate(rows_ub_chunks) if rows_ub_chunks else np.zeros(0, dtype=np.float64)
    )
...
def warm_run_synthetic_via_class(costs: np.ndarray, demands: np.ndarray) -> dict:
    ...
    for r in range(1, n_rolls):
        ...
        new_demand = fp.Param(("t",), pl.DataFrame({"t": t_idx, "value": demands[r]}))
        new_cost = fp.Param(("t",), pl.DataFrame({"t": t_idx, "value": costs[r]}))
...
def main_synthetic(n_rolls: int = 50, repeats: int = 3) -> None:
    ...
    print(
        f"  cold total (median):           {cold_med * 1000:9.1f} ms  "
    ...
    print(
        f"    rhs/cost updates:            {stats.median(warm_update) * 1000:9.1f} ms total "
        f"({stats.median(warm_update) / (n_rolls - 1) * 1000:7.3f} ms/roll)"
    )
    print(
        f"    h.run() time:                {stats.median(warm_solve) * 1000:9.1f} ms total "
    ...
    print(
        f"    rhs/cost updates:            {stats.median(wp_update) * 1000:9.1f} ms total "
        f"({stats.median(wp_update) / (n_rolls - 1) * 1000:7.3f} ms/roll)"
    )
    print(
        f"    h.run() time:                {stats.median(wp_solve) * 1000:9.1f} ms total "
    ...
    print("  obj equivalence:               OK")


def main_synthetic_large(n_rolls: int = 50, n_t: int = 168, repeats: int = 3) -> None:
    ...
    _N_T = n_t
    try:
        print(f"# Synthetic rolling-horizon chain (LARGER): {n_rolls} rolls of {n_t}h")
